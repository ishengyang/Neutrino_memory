...
\end{center}
\end{figure}
In this section we discuss how this effect can be also seen in traditional interferometers such as LIGO. We assume the geometry of the spacetime is governed by the SN progenitor star. We assume the star is not rotating very rapidly and thus the surrounding spacetime is parametrized by the Schwarzschild metric,
\begin{equation}
...
\begin{equation}
	\zeta^\mu_{f} = \left( \left( 1 - \frac{2M_f}{r_{crossing}} \right)^{-\frac{1}{2}}, v_i - \frac{\delta M}{r_{crossing}}, 0, 0 \right).	\label{5}
\end{equation}
Thus $v_f = v_i - \frac{\delta M}{r_{crossing}}$. Note that $\frac{\delta M}{r_{crossing}}$ is a coordinate velocity and to convert to proper velocity it will need to be multiplied by a factor of $\left( 1 - \frac{2M}{r_{crossing}} \right)^\frac{1}{2}$. $r_{crossing}$ is a fixed distance at which the shell crosses a point. For $A$ it is $r_0$, for $B$ it is $r_0 + d$ and for $C$ it is $r_0 + d + \mathcal{O}(d/r_0)$.
%
...
\section{Full interferometric calculation}

Distance change between two points can be measured by an interferometer. An interferometer calculates the distance by sending photons to a target and measuring the time it takes for them to come back. In this section we calculate how an interferometer will measure a distance change in its arms lengths when a shell of matter crosses it. We conisder points $A$, $B$ and $C$ in figure \ref{fig:1} as the ends of the arms of the interferometer. We parametrize the trajectories of the points in a piecewise form. For point $A$ we have

\begin{equation}
	r_A(t)  = \begin{cases} r_0 - \frac{1}{2} \frac{M}{r_0^2} t^2 & (t \leq 0), 	\\
	 r_0 - \frac{1}{2} \frac{M-\delta M}{r_0^2} t^2 - \frac{\delta M t}{r_0} \left( 1 - \frac{2(M -\delta M)}{r_0} \right)^\frac{1}{2} & (t>0).
	 \end{cases}	\label{rA}
\end{equation}
$t=0$ is defined to be the time when the shell crosses point $A$ and the distance $r_A(0) = r_0$. For $t<0$ we assume there is no velocity and the only term that contributes to the trajectory is the acceleration of point $A$ towards mass $M$. For $t>0$ there is still the acceleration term but the mass is different since the shell has carried $\delta M$ away. Along with that there is a term that corresponds to the velocity change coming from the shell crossing. In principle the $r_0$ on the RHS of Eq (\ref{rA}) should be $r_A(t)$ however we know $r_A(t) = r_0 + \mathcal{O}(M/r_0)$. Thus the effect of replacing $r_0$ with $r_A(t)$ will not affect the leading order answer. The trajectory of point $B$ is given by

\begin{equation}
	r_B(t) = \begin{cases} (r_0 + d)- \frac{1}{2}\frac{M}{(r_0+d)^2} t^2 &	(t \leq d), 	\\
	 (r_0 +d) - \frac{1}{2} \frac{M}{(r_0 + d)^2}d^2 - \frac{1}{2} \frac{M - \delta M}{(r_0 +d)^2}(t - d)^2 
	-  \frac{1}{2} \frac{\delta M}{r_0 + d}  \left( 1 - \frac{2(M -\delta M)}{r_0} \right)^\frac{1}{2} (t-d) - \frac{Md}{r_0^2}(t-d) &  (t>d).
	\end{cases}	\label{rB}
\end{equation}
For $t <d$ there is just the acceleration of the point towards mass $M$. After $t>d$ the shell has crossed and now there is a change in distance in the time $d$ (to leading order the shell takes time $d$ to go from $r_0$ to $r_0 + d$) that the point is accelerating towards $M$. This is the second term in the expression. The next term corresponds to the acceleration of the point towards mass $M-\delta M$. The next two terms contain the velocity terms for this point. The first term is the comes from the IJC. The second term is the velocity picked up by this point in the time the shell reaches it. Using these trajectories we can calculate how the distance between points $A$ and $B$ changes as a function of time. 

A photon is released at time $t$ after the shell has crossed point $A$. Using Eqs (\ref{rA}, \ref{rB}) we can calculate the proper time, $\tau$, it takes for this photon to go to point $B$ and come back to point $A$. The time taken, $\Delta t$, to travel between two points $r_A$ and $r_B$ is calculated by setting $ds = 0$ in Eq (\ref{SCH})
  
 \begin{equation}
 	\Delta t = \left( 1 -\frac{2M}{r_0} \right)^{-1} (r_B - r_A).
\end{equation}
Here we assume $r_0$ doesn't change to leading order. $r_B$ and $r_A$ will be time dependent in general. If we assume $r_B$ and $r_A$ change slowly in the time interval $\Delta t$ then we can simply evaluate $r_B$ at the time at the photon arrives and $r_A$ at the time the photon leaves (thus we don't have to integrate $r$ w.r.t time when solving Eq (\ref{SCH})). Converting the coordinate time interval to a proper time, $\Delta \tau$, gives

\begin{equation}
	\Delta \tau = \left( 1 - \frac{2M}{r_0} \right)^{- \frac{1}{2}} (r_B - r_A).
\end{equation}
We apply this equation to the trajectories in Eqs (\ref{rA}, \ref{rB}). The full calculation is given in the appendix. The proper time, $\tau$, it takes for a photon to go from $A$ to point $B$ and come back to $A$ (as shown in figure \ref{calculation}) is given by
 
 \begin{equation}
	 \tau = 2 d \left( 1 - \frac{M- \delta M}{r_0} + \frac{\delta M}{r_0} \right) + \mathcal{O}(r_0^{-2}).
\end{equation}
Note that there is no time dependence in this expression. The change in distance, $\Delta L_{AB}$, between the two points is given by subtracting the proper time it takes for a photon to complete this journey without any shell crossing

\begin{equation}
	\Delta L_{AB} = \tau - 2d \left( 1 - \frac{2M}{r_0} \right)^{\frac{1}{2}}= 4 d \frac{\delta M}{r_0} + \mathcal{O} (r_0^{-2}).
\end{equation}
Since this is a permanent change in the distance - it is nothing but a memory effect. There is another way to appreciate the lack of time dependence. We have assumed point $A$ has velocity zero at the time of shell crossing. After the shell crosses point $A$, it will pick up a velocity $\frac{\delta M}{r_0}$. In the time the shell takes to go from $A$ to $B$ (which is $d$ to leading order), $A$ will also gain a velocity from its acceleration towards the mass $M -\delta M$. The magnitude of the velocity gain from acceleration is $\frac{M-\delta M}{r_0^2} d$. The final velocity of $A$ at time $d$ is, therefore, given by

\begin{equation}
	v_A(t = d) = \frac{\delta M}{r_0} + \frac{M-\delta M}{r_0^2} d.
\end{equation}
...
\begin{equation}
	v_B(t=d) = \frac{\delta M}{r_0+d} + \frac{M}{(r_0+d)^2} d.
\end{equation}
Thus we see that $v_A$ is the same as $v_B$ to $\mathcal{O}(r_0^{-3})$ and therefore we don't expect the distance to change with time at leading order. Next we show an effect which is much more interesting since it grows with time.
\\
\\
...
Using $t_1$ and $t_2$ we get the proper time, $\tau$

\begin{equation}
	\tau = 2d \left( 1 - \frac{M - \delta M}{r_0} + \frac{\delta M}{r_0} \right).
\end{equation}
