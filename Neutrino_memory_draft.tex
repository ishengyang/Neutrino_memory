...
\end{equation}
Note that $\frac{\delta M}{r_{crossing}}$ is a coordinate velocity and to convert to proper velocity it will need to be multiplied by a factor of $\left( 1 - \frac{2M}{r_{crossing}} \right)^\frac{1}{2}$. $r_{crossing}$ is a fixed distance at which the shell crosses a point. For $A$ it is $r_E$, for $B$ it is $r_E + d$ and for $C$ it is $\approx r_E + d$ as well.\footnote{The exact expression is $r_E \left( 1 + \frac{2d}{r_0} + \frac{2d^2}{r_0^2} \right)^\frac{1}{2}$ but we are working to leading order so we can make the given approximation.}

\subsection{Distance change}

\subsubsection{The relationship between proper time and distance}\label{concept}
...
\subsubsection{Full interferometric calculation}
...
\begin{eqnarray}
	r_B(t) & = & r_0 + d - \frac{1}{2}\frac{M}{(r_0+d)^2} t^2,  \hspace{104mm}	(t \leq d) \nonumber	\\
	& = & r_0 +d - \frac{1}{2} \frac{M}{(r_0 + d)^2}d^2 - \frac{1}{2} \frac{M - \delta M}{(r_0 +d)^2}(t - d)^2 - \frac{1}{2} \frac{\delta M}{r_0 + d}  \left( 1 - \frac{2(M -\delta M)}{r_0} \right)^\frac{1}{2} (t-d). \hspace{10mm} (t>d)	\label{rB}
\end{eqnarray}
For $t <d$ there is just the acceleration of the point towards mass $M$. After $t>d$ the shell has crossed and now there is a change in distance in the time $d$ (to leading order the shell takes time $d$ to go from $r_0$ to $r_0 + d$) that the point is accelerating towards $M$. This is the second term in the expression. The other two terms correspond to the acceleration and velocity, respectively, of the point towards mass $M-\delta M$. Using these trajectories we can calculate how the distance between points $A$ and $B$ changes as a function of time. 
 ...
 \begin{equation}
	 \bar{\tau} = 2d - \frac{3 (M - \delta M) d}{r_0} + \frac{2 \delta M d}{r_0}.
\end{equation}
Note that there is no time dependence in this expression. In fact, the time dependence will come in at $\mathcal{O}(r_0^{-3})$ in this case.\footnote{See appendix where we show that there is no time dependence at second order.} As shown in section \ref{concept} this expression for $\tau$ will the leading order distance between $A$ and $B$. The change in distance after shell crossing is 

\begin{equation}
	\Delta L_{AB} \equiv \bar{\tau} - 2d = - \frac{3 (M - \delta M) d}{r_0} + \frac{2 \delta M d}{r_0}.
\end{equation}
...
\begin{eqnarray}
	\Delta L_{BC} &=& -\left( \frac{ \delta M}{r_0 + d} \left( 1 - \frac{2M}{r_0+d} \right)^\frac{1}{2}\sin \theta \right) t 
	\nonumber \\
	&=& - d \frac{\delta M}{r_0^2} t + \mathcal{O}(r_0^{-3})~.
\end{eqnarray}
Since they both fall toward the SN, they are getting closer to each other thus $\Delta L_{BC}$ is \emph{decreasing}. Thus we have found a memory effect that increases with time. This effect is suppressed compared to $\Delta L_{AB}$ by a factor of $r_0^{-1}$ but since it grows with time there will be times for which this effect will be dominant.
...
\begin{eqnarray}
	t_1 & = & d + \frac{1}{2} \frac{M-\delta M}{r_0^2} t^2 + \frac{\delta M}{r_0} \left( 1 - \frac{2(M-\delta M)}{r_0} \right)^\frac{1}{2} t - \frac{1}{2} \frac{M}{(r_0+d)^2} d^2 - \frac{1}{2} \frac{M- \delta M}{(r_0+d)^2} (t+t_1-d)^2 - \frac{\delta M}{r_0 + d} (t+t_1-d)	\nonumber	\\
	& - & \frac{M d}{r_0^2} (t + t_1 -d)	\nonumber	\\
	& = & d - \frac{1}{2} \frac{Md^2}{r_0^2} - \frac{M - \delta M}{r_0^2} d t + \mathcal{O}(r_0^{-3})
\end{eqnarray}

where we used $t_1 = d + \mathcal{O}(r_0^{-1})$. Similarly we calculate the time, $t_2$, it takes for a photon to go from $r_B$ to $r_A$. 

\begin{eqnarray}
	t_2 &  = & d + \frac{1}{2} \frac{M- \delta M}{r_0^2} (t + t_1 + t_2)^2 + \frac{\delta M}{r_0} \left( 1 - \frac{2(M- \delta M)}{r_0} \right)^\frac{1}{2} (t + t_1 + t_2) -  \frac{1}{2}\frac{M- \delta M}{(r_0 + d)^2} (t + t_1 - d)^2 \nonumber	\\
	& - & \frac{\delta M}{r_0 + d} \left( 1 - \frac{2(M- \delta M)}{r_0 + d} \right)^\frac{1}{2} (t + t_1 - d) - \frac{Md}{(r_0+d)^2} (t + t_1 -d )	\nonumber	\\
	& = & d + \frac{2 \delta M d}{r_0} + \frac{2 \delta M^2 d}{r_0^2} + 2 \frac{M- \delta M}{r_0^2} d^2 - 2 \frac{\delta M(M-\delta M) d}{r_0^2} +  \frac{M - \delta M}{r_0^2} dt + \mathcal{O}(r_0^{-3})
\end{eqnarray}
 
Lets convert these coordinate times into proper times, 

\begin{eqnarray}
	t_1' & = & \left( 1 - \frac{2(M- \delta M)}{r_0} \right) t_1	\nonumber	\\
	& = & d - \frac{M - \delta M}{r_0} d - \frac{1}{2} \frac{Md^2}{r_0^2} - \frac{M- \delta M}{r_0^2} dt	 + \mathcal{O}(r_0^{-3})
\end{eqnarray}

and for $t_2'$ we get

\begin{eqnarray}
	t_2' & = & \left( 1 - \frac{2(M- \delta M)}{r_0 + d} \right) t_2	\nonumber	\\
	& = & d + \frac{2 \delta M d}{r_0} + \frac{2 \delta M^2 d}{r_0^2} + \frac{2(M- \delta M) d^2}{r_0^2} + \frac{M - \delta M}{r_0^2} dt - \frac{2(M- \delta M) d}{r_0} - \frac{6 \delta M( M - \delta M) d}{r_0^2} + \frac{2 (M - \delta M) d^2}{r_0^2} + \mathcal{O}(r_0^{-3})	\nonumber	\\
\end{eqnarray}

giving the total travel time

\begin{equation}
	\bar{\tau} \equiv t_1' + t_2' = 2d - \frac{3 (M - \delta M) d}{r_0} + \frac{2 \delta M d}{r_0} - \frac{1}{2} \frac{M d^2}{r_0^2} + \frac{2 \delta M^2 d}{r_0^2} + \frac{4 (M-\delta M) d^2}{r_0^2} - \frac{6 \delta M (M- \delta M) d}{r_0^2} + \mathcal{O}(r_0^{-3})
\end{equation}

so we see there is no time dependence. 
