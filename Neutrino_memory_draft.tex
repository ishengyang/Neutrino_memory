...
\end{equation}
We take $\frac{M}{R_0^2}\ll 1$ and so we assume that $r$ doesn't change with time to do the integral,

\begin{equation}
	T-T_0 = R_B(T_0) - R_A(T_0) + 2M \ln \left( \frac{R_B(T_0) - 2M}{R_A(T_0) - 2M} \right).	\label{11}
\end{equation}
...
Now lets move on to the more realistic case of the interferometer shown in figure \ref{fig:2}. We parametrize the trajectory of each point in a piecewise form. For point $A$ we have
