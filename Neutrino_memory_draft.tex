...
\end{equation}
where are working in units with $G = c =1$. After the shell of energy released from the SN has passed the $i$ index is replaced by $f$ to denote the ``final" metric with a smaller mass $M _f = M_i -\delta M$. Since we are describing the shell as a delta function travelling at roughly the speed of light it will follow a null geodesic. The null vector of the shell can be written in both metrics as follows,
