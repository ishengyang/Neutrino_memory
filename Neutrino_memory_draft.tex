...
In particular the effect is calculated for photons from a pulsar, and the shell of matter is made of relativistic neutrinos from a nearby supernova.
The change in local geometry alters the path of photons.
...
This situation arises naturally in pulsar scintillation, which is caused by the signal propagating to us in multiple paths. 
...
Nevertheless, one can measure it as an interference effect, which can be much more accurate than pulsar timing \cite{PenYan14}.
We provide a simple estimation to show that it is likely observable by some of the pulsars that SKA is supposed to discover \cite{MSPpopulation}. 
...
We precede to clarify its relation and difference to the gravitational memory effect \cite{Christodoulou_effect,GW_memory} by computing the direct response from a interferometer.

%The recent detection of gravitational waves \cite{GW1509} has proved that gravitational waves leave an oscillating pattern in the amplitude of waveforms measured at interferometers such as LIGO. It is also known that this is not the only effect that is potentially detectable. 
...
Such a detection allow will allow us to derive the energy emitted by a supernova in neutrinos. Currently there is no other way to obtain such information. 
Thus in addition to direct neutrino detection experiments such as in Super-Kamiokande \cite{SuperKSN}, this memory effect can provide a new handle on constraining the explosion mechanism. 
...
In section \ref{sec-scint} we discuss how an astrophysical interferometer formed by pulsar scintillometry can measure the memory effect caused by a supernova. 
In section \ref{RelV} we derive the interferometer response in both transverse and longitudinal directions.  
In the final section \ref{obs} we discuss potential observation of such an effect by experiments that are currently being planned such as LISA and BBO.
