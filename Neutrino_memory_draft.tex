...
\begin{abstract}
When a supernova explodes, most of its energy are released in a neutrino shell which changes the surrounding geometry.
We calculate the potentially observable responses to such change from both pulsar scintillation and conventional interferometers.
In both cases, the responses are permanent changes to such transient event.
...
In the final section \ref{obs}, we put in reasonable values to estimate the size the interferometer response.
...
That means the inner product between the geodesic and the shell remains the same.\begin{equation}
	 g_{\mu \nu} u^\mu k^\nu = \bar{g}_{\mu \nu} \bar{u}^\mu \bar{k}^\nu. 	\label{0IJC}
\end{equation}
In order to satisfy this relation, this geodesic needs to pick up a velocity $v$.
...
\begin{eqnarray}
	...
	r_B(t) & = & r_0 +d  - \underbrace{\frac{1}{2} a_{(2)}^{(B)}(t-t_c)^2}_{T5} -  \underbrace{\frac{1}{2}  \bar{a}^{(B)}_{(2)} \bar{t}_c^2}_{T6} - \underbrace{\left( \bar{a}_{(2)}^{(B)} t_c + v^{(B)}_{(1)} + v^{(B)}_{(2)} \right)(t-t_c)}_{T7} ,  \hspace{5mm}  (t>t_c).	\label{rB}
\end{eqnarray}
...
%\footnote{After the shell crosses point $A$, it will pick up a velocity $\frac{\delta M}{r_0}\left( 1 + \frac{\delta M}{r_0} \right)$. In the time the shell takes to go from $A$ to $B$, $A$ will also gain a velocity from its acceleration towards the mass $M -\delta M$. The magnitude of the velocity gain from acceleration is $\frac{M-\delta M}{r_0^2} t_c$. The final velocity of $A$ at time $t_c$ is, therefore, given by
%\begin{equation}
%	v_A(t = t_c) = \frac{\delta M}{r_0} \left( 1 + \frac{\delta M}{r_0} \right)+ \frac{M-\delta M}{r_0^2} t_c.	\label{velA}
%\end{equation}
...
%	v_B(t=t_c) = \frac{\delta M}{r_0+d} \left( 1 + \frac{\delta M}{r_0} \right)+ \frac{M}{(r_0+d)^2} t_c.	\label{velB}
%\end{equation}
%We see that $v_A$ is the same as $v_B$ to $\mathcal{O}(r_0^{-2})$. } 
Next, we will calculate the distance between $A$ and $C$ there will be a relative velocity and therefore a time dependent turn. 
