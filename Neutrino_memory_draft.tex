...
\end{equation}
Note that $\frac{\delta M}{r_{crossing}}$ is a coordinate velocity and to convert to proper velocity it will need to be multiplied by a factor of $\left( 1 - \frac{2M}{r_{crossing}} \right)^\frac{1}{2}$. $r_{crossing}$ is a fixed distance at which the shell crosses a point. For $A$ it is $r_E$, for $B$ it is $r_E + d$ and for $C$ it is $\approx r_E + d$ as well.
...
We can combine the velocity change at each point with the acceleration at each point to calculate how the distance between the points changes. First lets consider the distance between the two points $A$ and $B$. The points are taken to have zero velocity at $t=0$. We can parametrize the trajectory of point $A$ as

\begin{equation}
	r_A(t)  = r_0 - \frac{1}{2} \frac{M-\delta M}{r_0^2} t^2 - \frac{\delta M t}{r_0} \left( 1 - \frac{2(M -\delta M)}{r_0} \right)^\frac{1}{2}.
\end{equation}

Similarly the trajectory of point $B$ is

\begin{equation}
	r_B(t) = r_0 + d - \frac{1}{2}\frac{M}{(r_0+d)^2}d^2 - \frac{1}{2} \frac{M- \delta M}{(r_0+d)^2} (t -d)^2 -\frac{\delta M}{r_0+d} \left( 1 - \frac{2(M -\delta M)}{r_0+d} \right)^\frac{1}{2}  (t-d) - \frac{M d}{(r_0+d)^2} (t -d).
\end{equation}

The first terms in this expression represents the distance at which the shell crosses. The next two terms represent the acceleration felt by point $B$ before and after shell crossing. The final line represents the velocity of $B$ before and after shell crossing. 

So we can compute the time, $t_1$, it takes for a photon, released at time $t$ after the shell has crossed, to go from $r_A$ to $r_B$. 
...
\begin{eqnarray}
	...
	& = & d + \frac{2 \delta M d}{r_0} + \frac{2 \delta M^2 d}{r_0^2} + 2 \frac{M- \delta M}{r_0^2} d^2 - 2 \frac{\delta M(M-\delta M) d}{r_0^2} +  \frac{M - \delta M}{r_0^2} dt
\end{eqnarray}
...
\begin{equation}
	t_1' + t_2' = 2d - \frac{3 (M - \delta M) d}{r_0} + \frac{2 \delta M d}{r_0} - \frac{1}{2} \frac{M d^2}{r_0^2} + \frac{2 \delta M^2 d}{r_0^2} + \frac{4 (M-\delta M) d^2}{r_0^2} - \frac{6 \delta M (M- \delta M) d}{r_0^2}
\end{equation}
