...
\end{eqnarray}
For $t <t_c$ there is just the acceleration of the point towards mass $M$. After $t>t_c$ the shell has crossed and now there is a change in distance in the time $t_c$ that the point is accelerating towards $M$. $t_c$ will be different in both metrics. However, since $\bar{t}_c$ will only come with acceleration terms (as the only thing that happens in the initial metric is acceleration), we can ignore the corrections for $\bar{t}_c$ and just approximate it with $d$. This is the second term in the expression. The next term corresponds to the acceleration of the point towards mass $M-\delta M$. The next two terms contain the velocity terms for this point. The first term comes from the IJC. The second term is the velocity picked up by this point in the time the shell reaches it. Using these trajectories we can calculate the proper time $\tau$ shown in figure \ref{calculation}. 
We know $t_c = d$ to leading order. Since we are interested in terms up to $ \mathcal{O}(r_0^{-2})$, we can substitute $t_c$ with $d$ in terms that are already $\mathcal{O}(r_0^{-2})$. 

In the appendix we show that this proper time, $\bar{\tau}_{AB}$ is the same as twice the proper length, $2 \bar{l}_{AB}$ measured between the two points at a \emph{fixed time} up to at least $\mathcal{O}(r_0^{-2})$, which is the order we are interested in. Thus we will simply compute the proper length between the two points $A$ and $B$, before and after shell crossing. The difference between the proper times will be the memory effect. The full calculation is in the appendix, the result is
...
First lets write down the trajectory equation for point $C$. 
...
The most general expression for the proper distance is again

\begin{equation}
	l_{AC} = \int \sqrt{g_{ab} dx^a dx^b}.
\end{equation}
...
We use the same result as above but with different trajectories for $r_B(t)$ and $r_A(t)$ now and in a metric defined by $M - \delta M$. 
...
Substituting in for $t_c = d \left( 1+ \frac{2(M - \delta M)}{r_0} \right)$ givs
...
Now we can compute the difference between the proper lengths before and after shell crossing

\begin{equation}
	\Delta l_{AB} = l_{AB} - \bar{l}_{AB} = - \frac{1}{2} \frac{\delta M^2 d}{r_0^2}.
\end{equation}
