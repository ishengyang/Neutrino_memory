...
In previous works \cite{PhysRevD.93.103006, Olum:2013gza} we and Olum et al have calculated the effect a shell of matter has upon crossing a pulsar. In particular the change observed in pulsar timing. The physical effect comes from the trajectory of a single photon changing due to a change in local geometry coming from the energy carried by the shell of matter. It is also known that many astrophysical objects scintillate due to scattering/lensing by objects in the interstellar medium. This scintillation can lead to multiple photons from the same object being observed. The geometric shift in each photon will depend on its distance to the object defining the geometry (in the same way as the amount of deflection a photon feels from a massive object depends on the impact parameter of the photon to that object). By considering photons that are parallel to each other we can construct an astrophysical 'interferometer' much like ground based interferometers such as LIGO.  

In this paper we calculate the effect from a matter shell crossing two geodesics. First for the case of two photons coming from pulsars, then for two freely falling points in ground based interferometers like LIGO. The particular matter shell we consider is a neutrino shell emitted by supernova. The shell crosses the path of light rays coming from a pulsar that is scintillating due to lensing in the interstellar medium. The shift in path lengths of both the photons lead to a permanent shift in the times at which they arrive. 
...
The rest of the paper is organized as follows. In section \ref{sec-scint} we discuss how an astrophysical interferometer formed by pulsar scintillometry can measure the memory effect caused by a supernova. In section \ref{RelV} we derive the change in velocities caused by the shell crossing the ends of an interferometer. This is done using the Israel Junction Conditions (IJC) \cite{Isr66} and by treating the neutrino shell as a co-dimension-one delta function. In the final section \ref{obs} we discuss potential observation of such an effect by experiments that are currently being planned such as LISA and BBO.
...
Throughout this calculation we assume that the interferometer is very far from the supernova. Thus we assume there are three small quantities, $\frac{M}{r_0}, \frac{\delta M}{r_0}, \frac{d}{r_0} \ll1$. To simplify notation we will use the symbol $\mathcal{O}(r^{-1})$ to represent suppression by any one of the three small quantities. 
...
An interferometer can measure the displacement between geodesics. We calculate the trajectories of the interferometer points and then integrate them to obtain the proper length. We will also show the proper length is related to the proper time as measured by one end of the interferometer arm. 

\subsubsection{Defining trajectories}

We consider points $A$, $B$ and $C$ in figure \ref{fig:1} as the ends of the arms of the interferometer. 
...
We have introduced new notation to represent the velocity and acceleration of the particles. We will also use this notation to describe other trajectories. To summarize this we list the terms below. 
...
\item $t=0$ is defined to be the time when the shell crosses point $A$ and the distance $r_A(0) = r_0$. For $t<0$ we assume there is no velocity and the only term that contributes to the trajectory is the acceleration of point $A$, $\bar{a}^{(A)}_{(2)}$ towards mass $M$. 

\item For $t>0$ there is still the acceleration term, $a^{(A)}_{(2)}$, but the mass is different since the shell has carried $\delta M$ away. 

\item Along with that there is a term that corresponds to the velocity change $(v^{(A)}_{(1)} + v^{(A)}_{(2)})$ (as we calculated from the IJC) coming from shell crossing. 
...
The interferometer infers the distance between two points by measuring the proper time it takes for a photon to go from one end of the arm to the other and come back. We know the velocity induced by shell crossing is small, i.e $\mathcal{O}(r_0^{-1})$. We will use this to show that \emph{the proper time seen by observer $A$, $\tau$, is equal to twice the proper length $l$ of the interferometer arm up to $\mathcal{O}(r_0^{-2})$}.

 The most general expression for the proper distance for a stationary observer is
...
After shell crossing the two points will have no relative velocity between them (schematic diagram of the velocities is shown in figure \ref{velocity}) up to order $\mathcal{O}(r_0^{-2})$. Therefore after shell crossing we can also just calculate the proper length $l_{AB}$. First lets show that there is no relative velocity between $A$ and $B$ explicitly. 
